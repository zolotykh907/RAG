...
class Indexing:
    ...
    def check_existing_hashes(self):
        ...
        if os.path.exists(self.hashes_path):
            ...
            self.existing_hashes = pd.DataFrame(columns=['hash'])
    ...
    def update_existing_hashes(self, df):
        ...
        self.existing_hashes = pd.concat([self.existing_hashes, pd.DataFrame({'hash': new_hashes})], ignore_index=True)
    ...
    def run_indexing(self):
        ...
        new_hashes_df = df_chunks_new[['text_hash']].rename(columns={'text_hash': 'hash'})
